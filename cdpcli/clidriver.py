...
#        conftest_cmd = DockerCommand(self._cmd, self._context.opt['--docker-image-conftest'], "%s:/project" % chartdir, True)
        ...
        self._cmd.run('docker run --rm -e DOCKER_HOST --entrypoint="ls" -v /var/run/docker.sock:/var/run/docker.sock --volumes-from $(docker ps -aqf "name=k8s_build_${HOSTNAME}") instrumenta/conftest:v0.18.2 -Rl' % chartdir)
        self._cmd.run('docker run --rm -e DOCKER_HOST --entrypoint="ls" -v /var/run/docker.sock:/var/run/docker.sock --volumes-from $(docker ps -aqf "name=${HOSTNAME}-build") instrumenta/conftest:v0.18.2 -Rl' % chartdir)
