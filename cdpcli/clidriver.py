...
Options:
    ...
    --DaltDeploymentRepository                                 Use custom Maven Dpeloyement repository
    ...
    def __maven(self):
        ...
        if self._context.opt['--deploy']:
            if self._context.opt['--deploy'] == 'release':
                ...
                if self._context.opt['--DaltDeploymentRepository']:
                    arguments = '-DskipTests -DskipITs -Dproject.scm.id=git -DaltDeploymentRepository=%s' % (self._context.opt['--DaltDeploymentRepository'])
                else:
